...
db_config = {
    ...
    'user': 'postgres',
    'password': 'postgres',
    'host': 'db',  # Docker服务名称或容器的IP地址
    'port': '5432'
}

def search_in_database(embedding, top_k=10, threshold=0.5):
...
if __name__ == "__main__":
    ...
    # model.load_state_dict(torch.load('checkpoint/autoencoder.pth'))
    # model.eval()
    ...
    embedding=object_embedding(input_image).tolist()
    ...
    for result in results:
        ...
        if frame_array is not None:
            ...
            cv2.imwrite(f'result_img/index_{i}.jpg', drawn_frame)
